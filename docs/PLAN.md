...
- [ ] User fills in `.env` values (DATABASE_URL, Clerk keys, Plaid keys, NEXT_PUBLIC_API_URL)
- [ ] First git commit — checkpoint the current scaffold before any fixes land, so "before"
      is preserved for the audit story / CV diff
- [ ] `npm run dev` confirmed working locally against real `.env`

## Phase 1 — Test infrastructure
- [ ] Install vitest (+ `@vitest/coverage-v8` optional), add `vitest.config.ts`, add
      `"test": "vitest run"` / `"test:watch": "vitest"` to package.json scripts
- [ ] First suite: transaction ownership/IDOR tests (mocked `db`, no live DB needed) — see
      docs/BUGS.md BUG-001/BUG-002 and the test list below
- [ ] Run the suite. Expect BUG-001/BUG-002 tests to fail red — that's the point, it proves
      the bug instead of assuming it
- [ ] Any *new* bug a test surfaces gets appended to docs/BUGS.md with the next BUG-0xx
      number, in the priority section that matches its severity, before moving on
...
## Ongoing
- `/tekshir` (project-scoped skill, `.claude/skills/tekshir/SKILL.md`) reviews commits since
  the last docs/PROGRESS.md update against this plan and docs/BUGS.md, runs the checks, and
  produces a ready-to-paste prompt for whichever session does the actual fixing.
